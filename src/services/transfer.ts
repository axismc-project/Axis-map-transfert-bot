...
export class TransferService {
  ...
  constructor(
    private srv1Config: ServerConfig,
  ...
  async executeTransfer(progressCallback?: (tracker: ProgressTracker) => void): Promise<void> {
    ...
    try {
      ...
      await this.handleRollback(srv1Connected, srv2Connected);
      throw error;
  ...
  private async handleRollback(srv1Connected: boolean, srv2Connected: boolean): Promise<void> {
    try {
